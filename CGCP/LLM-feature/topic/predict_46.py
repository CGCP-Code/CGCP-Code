...
df['stay_46'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_46.csv", index=False)

print(f"Total positive classifications: {df['stay_46'].sum()}")
print(f"Total negative classifications: {(df['stay_46'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_46'].sum() / len(df)) * 100:.2f}%")