...
def classify_conversation(row):
    ...
    topic = "Increase cauliflower yield"
...
df['stay_19'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_19.csv", index=False)

print(f"Total positive classifications: {df['stay_19'].sum()}")
print(f"Total negative classifications: {(df['stay_19'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_19'].sum() / len(df)) * 100:.2f}%")