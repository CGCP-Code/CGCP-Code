...
df['stay_2'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_2.csv", index=False)

print(f"Total positive classifications: {df['stay_2'].sum()}")
print(f"Total negative classifications: {(df['stay_2'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_2'].sum() / len(df)) * 100:.2f}%")