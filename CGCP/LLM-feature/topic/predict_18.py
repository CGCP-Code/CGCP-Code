...
df['stay_18'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_18.csv", index=False)

print(f"Total positive classifications: {df['stay_18'].sum()}")
print(f"Total negative classifications: {(df['stay_18'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_18'].sum() / len(df)) * 100:.2f}%")