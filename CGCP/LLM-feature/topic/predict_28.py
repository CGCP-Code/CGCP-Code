...
def classify_conversation(row):
    ...
    topic = "Seed purification methods"
...
df['stay_28'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_28.csv", index=False)

print(f"Total positive classifications: {df['stay_28'].sum()}")
print(f"Total negative classifications: {(df['stay_28'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_28'].sum() / len(df)) * 100:.2f}%")