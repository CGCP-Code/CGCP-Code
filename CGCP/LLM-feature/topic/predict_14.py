...
def classify_conversation(row):
    ...
    topic = "practices for intercropping"
...
df['stay_14'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_14.csv", index=False)

print(f"Total positive classifications: {df['stay_14'].sum()}")
print(f"Total negative classifications: {(df['stay_14'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_14'].sum() / len(df)) * 100:.2f}%")