...
def classify_conversation(row):
    ...
    topic = "Rainwater harvesting systems"
...
df['stay_24'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_24.csv", index=False)

print(f"Total positive classifications: {df['stay_24'].sum()}")
print(f"Total negative classifications: {(df['stay_24'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_24'].sum() / len(df)) * 100:.2f}%")