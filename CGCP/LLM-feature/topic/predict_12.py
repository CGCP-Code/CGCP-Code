...
def classify_conversation(row):
    ...
    topic = "Improve threshing techniques"
...
df['stay_12'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_12.csv", index=False)

print(f"Total positive classifications: {df['stay_12'].sum()}")
print(f"Total negative classifications: {(df['stay_12'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_12'].sum() / len(df)) * 100:.2f}%")