...
def classify_conversation(row):
    ...
    topic = "Impact of humidity on crops"
...
df['stay_31'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_31.csv", index=False)

print(f"Total positive classifications: {df['stay_31'].sum()}")
print(f"Total negative classifications: {(df['stay_31'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_31'].sum() / len(df)) * 100:.2f}%")