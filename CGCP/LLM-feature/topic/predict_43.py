...
df['stay_43'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_43.csv", index=False)

print(f"Total positive classifications: {df['stay_43'].sum()}")
print(f"Total negative classifications: {(df['stay_43'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_43'].sum() / len(df)) * 100:.2f}%")