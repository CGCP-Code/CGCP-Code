...
df['stay_26'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_26.csv", index=False)

print(f"Total positive classifications: {df['stay_26'].sum()}")
print(f"Total negative classifications: {(df['stay_26'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_26'].sum() / len(df)) * 100:.2f}%")