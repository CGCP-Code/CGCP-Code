...
def classify_conversation(row):
    ...
    topic = "Treatment for wheat seed"
...
df['stay_50'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_50.csv", index=False)

print(f"Total positive classifications: {df['stay_50'].sum()}")
print(f"Total negative classifications: {(df['stay_50'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_50'].sum() / len(df)) * 100:.2f}%")