...
def classify_conversation(row):
    ...
    topic = "Disease prevention rainy"
...
df['stay_49'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_49.csv", index=False)

print(f"Total positive classifications: {df['stay_49'].sum()}")
print(f"Total negative classifications: {(df['stay_49'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_49'].sum() / len(df)) * 100:.2f}%")