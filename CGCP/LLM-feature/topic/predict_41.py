...
def classify_conversation(row):
    ...
    topic = "Finding certified seed suppliers"
...
df['stay_41'] = df.progress_apply(classify_conversation, axis=1)

# Save results
df.to_csv("data_with_stay_41.csv", index=False)

print(f"Total positive classifications: {df['stay_41'].sum()}")
print(f"Total negative classifications: {(df['stay_41'] == 0).sum()}")
print(f"Percentage positive: {(df['stay_41'].sum() / len(df)) * 100:.2f}%")