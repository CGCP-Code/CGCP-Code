...
for SEED in range(924, 934):
    ...
    for epoch in range(1, 3):
        ...
        if current_f1 > best_f1 + 1e-4:
            best_f1, no_improve = current_f1, 0
            torch.save(mlp_model.state_dict(), f"best_mlp_seed_{SEED}.pth")
        else:
            no_improve += 1
    ...
    mlp_model.load_state_dict(torch.load(f"best_mlp_seed_{SEED}.pth", map_location=device))
    mlp_model.eval()
    ...
    # Calculate status for each row using combined train+test data
